...
CREATE TABLE "MST_FUSION_ROLES_ARCHIVE" 
(	"ID" NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER  NOCYCLE  NOKEEP  NOSCALE  NOT NULL ENABLE, 
"ROLE_GUID" NVARCHAR2(10000), 
"ROLE_NAME" VARCHAR2(4000 CHAR), 
"ROLE_DESC" NVARCHAR2(10000), 
"IS_ACTIVE" VARCHAR2(10 CHAR) DEFAULT 'Y', 
"SYNC_ID" NVARCHAR2(10000), 
    CONSTRAINT "MST_FUSION_ROLES_ARCHIVE_PK" PRIMARY KEY ("ID")
USING INDEX  ENABLE
) ;
...
SELECT MST_ROLE_CATEGORY_MAP.ID,
       MST_FUSION_ROLES.ROLE_NAME,
       MST_FUSION_ROLES.ROLE_DESC,
       APPLICATION_CATEGORY_MST.TITLE,
       MST_ROLE_CATEGORY_MAP.FINANCIAL_IMPACT,
       MST_ROLE_CATEGORY_MAP.MASTER_DATA_IMPACT,
       MST_ROLE_CATEGORY.CATEGORY_NAME,
       MST_ROLE_CATEGORY_MAP.IS_ACTIVE,
       MST_ROLE_CATEGORY_MAP.CREATED_AT,
       MST_ROLE_CATEGORY_MAP.CREATED_BY,
       MST_ROLE_CATEGORY_MAP.UPDATED_AT,
       MST_ROLE_CATEGORY_MAP.UPDATED_BY
FROM MST_ROLE_CATEGORY_MAP
LEFT JOIN MST_ROLE_CATEGORY ON MST_ROLE_CATEGORY_MAP.CATEGORY_ID=MST_ROLE_CATEGORY.ID
LEFT JOIN MST_FUSION_ROLES ON MST_ROLE_CATEGORY_MAP.ROLE_ID=MST_FUSION_ROLES.ID
LEFT JOIN APPLICATION_CATEGORY_MST ON MST_ROLE_CATEGORY_MAP.APPLICATION_CATEGORY= APPLICATION_CATEGORY_MST.ID
...
WITH RECURSIVE split_roles AS (
    -- Anchor member: split the first role
    SELECT
        ID,
        TRIM(SPLIT_PART(ROLE_ID, ':', 1)) AS ROLE_ID,
        -- Remove the first role and leading colon
        CASE WHEN POSITION(':' IN ROLE_ID) > 0 THEN SUBSTRING(ROLE_ID FROM POSITION(':' IN ROLE_ID) + 1) ELSE '' END AS REMAINING_ROLES
    FROM
        MST_ROLE_CATEGORY_MAP
    UNION ALL
    -- Recursive member: split the next role
    SELECT
        ID,
        TRIM(SPLIT_PART(REMAINING_ROLES, ':', 1)) AS ROLE_ID,
        -- Remove the next role and leading colon
        CASE WHEN POSITION(':' IN REMAINING_ROLES) > 0 THEN SUBSTRING(REMAINING_ROLES FROM POSITION(':' IN REMAINING_ROLES) + 1) ELSE '' END
    FROM
        split_roles
    WHERE
        REMAINING_ROLES <> ''
)
-- Insert the result into Table_B
INSERT INTO MST_ROLE_CATEGORY_MAP_TEST (ID, ROLE_ID, CATEGORY_ID, IS_ACTIVE, CREATED_AT, CREATED_BY, UPDATED_AT, UPDATED_BY, FINANCIAL_IMPACT, MASTER_DATA_IMPACT, APPLICATION_CATEGORY)
SELECT
    ID, 
	ROLE_ID, 
	CATEGORY_ID, 
